...
\end{itemize}
\pause
  $\Rightarrow$ La programmation sur GPU est {\it grosso modo } donc un parallèlisme à {\bf grain fin}, à mémoire {\bf partagée} tournant sur
...
\begin{minipage}[c]{0.49\linewidth}
  \begin{itemize}[<+->]
    ...
    \item Tesla K40M : 
      \begin{itemize}
        \item 15 SM
        \item 192 x 15 = 2880 SP
        \item 2048 threads max par SM = 30720 threads
      \end{itemize}
    \item ...pas au sens classique
      \begin{itemize}
        \item parallelisme de données respectant des motifs d'accès réguliers
      \end{itemize}
  \end{itemize}
\end{minipage}
\begin{minipage}[c]{0.49\linewidth}
