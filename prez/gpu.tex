...
\begin{frame}{Réduction}{2 niveaux} 
 \begin{itemize}[<+->]
   \item On va écrire 2 noyaux
 \begin{itemize}
   \item {\tt partialSum} qui dénombre les couples $(x_i,y_i)$ dans $A$ pour chaque bloc. On stocke ce résultat
      en mémoire globale
    \item {\tt finalSum} qui calcule la somme totale des résultats précedents
 \end{itemize}
   \item[\kontuz] agrégation d'une information commune par bloc \gezi\ mémoire partagée
  \end{itemize}
\pause
\begin{center}
...
\pause
  \begin{itemize}[<+->]
    \item On découpe les données par paquet de taille {\tt gridDim.x * blockDim.x}
    \item[ach
