...
\begin{frame}{Organisation des threads}{découpage des données}
 ...
 \begin{minipage}[c]{0.59\linewidth}
  \begin{itemize}[<+->]
    ...
    \item on paramètre l'appel du noyau \lstinline!myKernel<<<grille,blocs>>>(...)! avec la taille des blocs et de la grille
  \end{itemize}
\end{minipage}
\begin{minipage}[c]{0.39\linewidth}
\begin{center}
...
\begin{minipage}[c]{0.59\linewidth}
 type CUDA pour gérer les dimensions 
    \begin{itemize}
      \item \alert{dim3} est une structure avec 3 champs entiers \texttt{\_.x,\_.y et \_.z}
      \item descripteur de grille (dg) et de bloc (db)
    \end{itemize}
 Limites de bloc 3D
   \begin{itemize}
     \item \texttt{db.x}, \texttt{db.y} $\leqslant 1024$, \texttt{db.z} $\leqslant 64$ 
     \item nb total de threads / bloc $\leqslant 1024$
     \item \texttt{db.z} = 1 $\mapsto$ 2D
     \item \texttt{db.z} = \texttt{db.y} = 1 $\mapsto$ 1D
   \end{itemize}
 limites de la grille 3D
   \begin{itemize}
     \item \texttt{dg.x} $\leqslant 2^{31}-1$, \texttt{dg.y}, \texttt{dg.z} $\leqslant 2^{16}-1$
     \item nb total de threads / bloc $\leqslant 1024$
     \item \texttt{dg.z} = 1 $\mapsto$ 2D
     \item \texttt{dg.z} = \texttt{dg.y} = 1 $\mapsto$ 1D
   \end{itemize}
\end{minipage}
\begin{minipage}[c]{0.39\linewidth}
